...
import { updateWorkspaceSchema } from "@/zodSchemas";
...
function editWorkspaceForm({
  ...
  const { setWorkspaceId, workspaceId } = useWorkspaceId();
  ...
  const { mutateAsync: updateWorkspaceAsync } = useMutation({
    mutationFn: useConvexMutation(api.workspaces.updateById),
    onSuccess: () => {
  ...
  const handleDelete = async () => {
    ...
    if (!ok) return;
    deleteWorkspace({ id: workspaceId! as Id<"workspaces"> });
  };
